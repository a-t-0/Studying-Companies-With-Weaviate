...
class WebsiteHierarchy:
  ...
  """
  def __init__(self):
    self.hierarchy = defaultdict(WebsiteHierarchy)  # Nested dictionaries for hierarchy
  ...
  def add_url(self, url):
    ...
    self.hierarchy[path_components[0]].add_url("/".join(path_components[1:]))
...
def build_hierarchy_v0(urls:List[str]):
  ...
  return {"weaviate.io": hierarchy}  # Assuming "weaviate.io" is the root domain
...
def build_hierarchy(urls:List[str], root:str, hierarchy=None, path=[]):
  ...
  hierarchy[current_path[0]] = build_hierarchy("/".join(path_components), hierarchy.copy(), current_path)
...
def add_url_to_dict(url_structure: dict, url_remainder: str, current_path: list[str]) -> None:
    ...
    add_url_to_dict(url_structure[new_key], remaining_path, current_path + [new_key])


def add_nested_dict_entry(some_dict: dict, current_path: list[str], new_key: str, new_value):
    d = some_dict
