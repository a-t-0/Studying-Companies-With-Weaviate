...
folderDropdown.addEventListener("change", () => {
	...
	console.log("IN TREE selectedFolder.value=", selectedFolder.value);
	...
	function Tree(
		...
		node
			...
			.attr("r", r);
		node.append("title").text((d) => d.data.title); // Assumes that your data has a 'title' property
	...
	const url =
		"http://0.0.0.0:8000/output_data/" + selectedFolder.value + "/d3_data.json";
	// Clear the container.
	container.innerHTML = "";

	// Get the data from the local Python server.
	fetch(url)
